...
The copy number difference has no significant effect in the mESC data (p = `r format(summary(dm)$coefficients[2,4], digits = 3)`, linear model), though is significant in the Deng data (p = `r format(summary(dd)$coefficients[2,4], digits = 2)`, linear model). However, here the coefficient value is extremely small (`r format(coef(dd)[2], digits = 3)` compared to intercept of `r format(coef(dd)[1], digits = 3)`).
